...
# Date: 2016.5.19 Version 1.3
# Adjustment: The order of reading individual Q trace one by one to form comglomerated Q file should be modified for further operation. Instead of reading individuals in alphabetic order (default reading order), we should read the trace in reverse alphabetic order. (With Z component of trace being read first, N following Z, and E being read lastly.) The "tac" command is applied for this purpose(print files in reverse).

Workdir=/home/hep
SACdatadir=${Workdir}/Nepal_SAC_Processed/YL
Qdatadir=${Workdir}/Nepal_Q_YL
infodir=${Workdir}/Info_Nepal_YL
...
#cd $infodir
#cat Nepal_Stations_YL | awk '{print $2,"   lat:",$7,"   lon:",$8,"   elevation:",$9}' >> ${shdir}/inputs/STATINF.DAT
#cat Nepal_Stations_YL | awk '{print $2"-bh-?...                ...                 1.0"}' | tr 'A-Z' 'a-z' >> ${shdir}/inputs/sensitivities.txt
#cat Nepal_Stations_YL | awk '{print $2"-BH-Z "$1}{print $2"-BH-N "$1}{print $2"-BH-E "$1}' >> ${shdir}/inputs/filter_lookup.txt
...
for stat in `ls |grep -v 000D`
do
Qstat=${Qdatadir}/${stat}
SACstat=${SACdatadir}/${stat}
mkdir -p $Qstat
cd $SACstat
saclist=`ls |grep SAC.new`
...
#MODIFICATION change the variable sh_file name designation from ":r" to "_r" 
sh_file=$shasc_file"_r"
...
cd $infodir
eventtime=`cat Nepal_Index_YL |grep "$station $startyr $startmon $startday $starthr $startmin" | awk '{print $8}'`
...
evtinfo=`cat Nepal_Events_YL |grep "$evtyr $evtmon $evtday $evthr $evtmin $evtsec" | awk '{print $7"_"$8"_"$9"_"$10}'`
...
slat=`cat Nepal_Stations_YL | grep $station | awk '{print $7}'`
slon=`cat Nepal_Stations_YL | grep $station | awk '{print $8}'`
elev=`cat Nepal_Stations_YL | grep $station | awk '{print $9}'`
...
for file in `ls |grep QBN |sed 's/.QBN//' |tac`
do
